            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>¥{{ "{:,.0f}".format(p.cash) }}</td>
                        <td class="{% if p.total_floating_pnl >= 0 %}positive{% else %}negative{% endif %}">
                            {{ "+" if p.total_floating_pnl >= 0 else "" }}¥{{ "{:,.0f}".format(p.total_floating_pnl) }}
                        </td>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ pos.name }}</td>
                        <td>{{ "{:,.0f}".format(pos.shares|float) }}股</td>
                        <td>¥{{ "{:.2f}".format(pos.avg_cost|float) }}</td>
                        <td>¥{{ "{:.2f}".format(pos.current_price|float) }}</td>
                        <td>¥{{ "{:,.0f}".format(pos.market_value|float) }}</td>
                        <td class="{% if pos.floating_pnl >= 0 %}positive{% else %}negative{% endif %}">
                            {{ "+" if pos.floating_pnl >= 0 else "" }}¥{{ "{:,.0f}".format(pos.floating_pnl|float) }}
                        </td>
                        <td class="{% if pos.floating_pnl_pct >= 0 %}positive{% else %}negative{% endif %}">
                            {{ "+" if pos.floating_pnl_pct >= 0 else "" }}{{ "{:.2f}".format(pos.floating_pnl_pct|float) }}%
                        </td>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ "{:,.0f}".format(trade.shares|float) }}</td>
                        <td>¥{{ "{:.2f}".format(trade.price|float) }}</td>
                        <td>¥{{ "{:,.0f}".format(trade.amount|float) }}</td>
                        <td class="{% if trade.pnl >= 0 %}positive{% else %}negative{% endif %}">
                            {{ "+" if trade.pnl >= 0 else "" }}¥{{ "{:,.0f}".format(trade.pnl|float) }}
                        </td>
                        <td>{{ trade.reason }}</td>
                    </tr>
                    {% endfor %}
...
def calculate_signals(positions_data, config):
    ...
    for pos in positions_data:
        ...
        # 检查信号
        stop_loss = config.get('stop_loss', 0.05) * 100  # 转为百分比
        ...
        pos['current_price'] = round(float(current_price), 2)
        pos['market_value'] = round(float(shares) * current_price, 0)
        pos['floating_pnl'] = round(float((current_price - avg_cost) * shares), 0)
        pos['floating_pnl_pct'] = round(float(pnl_pct), 2)
        pos['hold_days'] = int(hold_days)
        pos['shares'] = shares  # 确保shares也是整数
        pos['avg_cost'] = round(float(avg_cost), 2)
...
def api_positions(portfolio_id):
    ...
    try:
        ...
        return jsonify({'success': True, 'data': positions.data})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
...
def api_trades():
    ...
    if request.method == 'POST':
        ...
        # 获取交易记录
        try:
            trades = supabase.table('trades').select('*').order('trade_date', desc=True).limit(50).execute()
            return jsonify({'success': True, 'data': trades.data})
        except Exception as e:
            return jsonify({'success': False, 'error': str(e)})
